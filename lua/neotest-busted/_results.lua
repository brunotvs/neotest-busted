...
local function decode_result_output(path)
  local marker =
    require('neotest-busted._output-handler').NEOTEST_RESULTS_LINE_MARKER
  local lines = vim.fn.readfile(path)

  local line = vim
    .iter(lines)
    :find(function(l)
      return vim.startswith(l, marker)
    end)
    :sub(#marker + 1)
  ...
  for _, err in ipairs(errors) do
    local content = {
      ('Error -> %s @ %d'):format(
        err.element.trace.short_src,
        err.element.trace.currentline
      ),
      err.name,
    }
    writefile(content, path, 'Sa')
